...
In this study, gas chromatography-atmospheric pressure chemical ionization-ion mobility-high resolution mass spectrometry (GC-APCI-IM-HRMS) was employed to develop a suspect screening workflow constrained by full scan mass spectrum (i.e., MS<sup>1</sup>), retention time (RT), collision cross section (CCS), and fragmentation spectrum (i.e., MS<sup>2</sup>), alongside a continuous scoring system, to reduce false positives and improve identification confidence.

## Suspect lists: 
We compiled two suspect lists containing information on identity, exact mass, RT, and CCS values.
